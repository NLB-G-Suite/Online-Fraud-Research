...
for f_index in range(29,50):
	try:
		chanVideos=json.load(open('scaledChan'+str(f_index)+'.json'))
	except:
...
# def classify(chanVideos,xTrain1,y_train):
	...
	with open('classifyScaledChan'+str(f_index)+'.json', 'w') as fp:
	    json.dump(chanVideos, fp)

	clasChan=json.load(open('classifyScaledChan'+str(f_index)+'.json'))
